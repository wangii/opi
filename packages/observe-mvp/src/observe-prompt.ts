export const OBSERVE_TOOL_DESCRIPTION =
	"Use observe only when the current semantic framing may be hiding a materially different next action, or when the active frame no longer makes preserving and retrieving task-relevant information cheaper than equivalent raw context. Record a concise provisional frame without copying evidence already available through source references. After observing, continue with one bounded action influenced by the observation, unless user input is genuinely required.";
